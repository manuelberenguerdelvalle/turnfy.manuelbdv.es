...
?>
  ...
  <div class="content-wrapper">
    <?php pageHeader("Actividad Semanal"); ?>
    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
        ...
        if( isset($_POST['dia']) && !empty($_POST['dia']) ){
          ...
          //carga de datos
          if(!isset($_SESSION['semana_actividad'][0])){
            //echo 'if2';
            $dia = date('Y-m-d');
          }
          else{
            $dia = $_SESSION['semana_actividad'][0];
          }
        }
        ...
        $_SESSION['semana_min_actividad'] = date("Y-m-d",strtotime($fecha_min_actividad));
        ...
        $_SESSION['semana_max_actividad'] = date("Y-m-d",strtotime($fecha_max_actividad));

        actividades_semanal($id_empresa,$dia);//para cargar el mes

        $semana_anterior = date("Y-m-d",strtotime($_SESSION['semana_actividad'][0]." - 1 week"));
        $semana_posterior = date("Y-m-d",strtotime($_SESSION['semana_actividad'][0]." + 1 week"));

        $meses = obten_nombreMeses();
        $tit_desde = $meses[date("n",strtotime($_SESSION['semana_actividad'][0]))-1];
        $titulo_dia1 = substr(nombre_dia_semana(date("N",strtotime($_SESSION['semana_actividad'][0]))),0,2).' '.date("d/m/Y",strtotime($_SESSION['semana_actividad'][0]));
        $titulo_dia2 = substr(nombre_dia_semana(date("N",strtotime($_SESSION['semana_actividad'][1]))),0,2).' '.date("d/m/Y",strtotime($_SESSION['semana_actividad'][1]));
        ...
        <div class="row">
          ...
          <div class="form-group col-sm-4">
            <form role="form" id="formDia" method="POST" action="actividad_semanal.php">
              <div class="input-group">
                ...
                <input type="date" name="dia" max="<?= $_SESSION['semana_max_actividad'] ?>" min="<?= $_SESSION['semana_min_actividad'] ?>" class="form-control" value="<?= $dia ?>" onchange="submit()">
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <div class="col-1">
            <?php
            if($_SESSION['semana_actividad'][0] >= $_SESSION['semana_min_actividad'] ){
            ?>
          ...
          <div class="col-1">
            <?php
            if($semana_posterior <= $_SESSION['semana_max_actividad']){
            ?>
            <form role="form" id="anterior" method="POST">
          ...
          <div class="col-12"
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Actividad <?= $_SESSION['actividades_total_semanal'].' registros en total esta semana' ?></h3>
...
<script>
  $(function () {
    ...
    var donutData        = {
      labels: [
        <?= $_SESSION['noms_usuarios_semanal']; ?>
        /*
          'Chrome', 
          'IE',
          'FireFox', 
      ...
      datasets: [
        {
          ...
          data: [<?= $_SESSION['datos_usuarios_semanal'] ?>],
          //data: [700,500,400,600,300,100],
          
          backgroundColor : [<?= $_SESSION['colores_semanal'] ?>],
