...
?>
  ...
  <div class="content-wrapper">
    <?php pageHeader("Estadísticas Semanales"); ?>
    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
        ...
        if( isset($_POST['dia']) && !empty($_POST['dia']) ){
          $dia = $_POST['dia'];
          unset($_SESSION['horarios_semanal']);
          estadisticas_total_semanal($id_empresa,$dia);//para cargar la semana
        }
        else{
          //carga de datos
          if(!isset($_SESSION['dia1_total_semanal'][0])){
            $dia = date('Y-m-d');
            estadisticas_total_semanal($id_empresa,$dia);//para cargar la semana
          }
          else{
            $dia = $_SESSION['dia1_total_semanal'][0];
          }
        }
        ...
        //$_SESSION['diax_total_semanal'][0] // guardamos la fecha desde/hasta
        //$_SESSION['diax_total_semanal'][1] // guardamos los turnos
        //$_SESSION['diax_total_semanal'][2] // guardamos las ganancias
        //$_SESSION['diax_total_semanal'][3] // guardamos las asistencias
        ...
        for($i=1; $i <= 7; $i++){
          if($_SESSION['dia'.$i.'_total_semanal'][0] != ''){
            $fecha_completa = nombre_dia_semana(date("N",strtotime($_SESSION['dia'.$i.'_total_semanal'][0]))).' '.date("d",strtotime($_SESSION['dia'.$i.'_total_semanal'][0]));
            if($_SESSION['dia'.$i.'_total_semanal'][1] > 0){ $data_turnos .= $_SESSION['dia'.$i.'_total_semanal'][1]."-"; $dias_turnos[$cont1] = $fecha_completa; $cont1++; }
            if($_SESSION['dia'.$i.'_total_semanal'][2] > 0){ $data_ganancias .= $_SESSION['dia'.$i.'_total_semanal'][2]."-"; $dias_ganancias[$cont2] = $fecha_completa; $cont2++; }
            if($_SESSION['dia'.$i.'_total_semanal'][3] > 0){ $data_asistencias .= $_SESSION['dia'.$i.'_total_semanal'][3]."-"; $dias_asistencias[$cont3] = $fecha_completa; $cont3++; }
          }
        }//fin for

        for($i = 0; $i < 7; $i++){//vaciamos las semanas que no tienen nada
          if(empty($dias_turnos[$i])){
            $dias_turnos[$i] = '';
          }
          if(empty($dias_ganancias[$i])){
            $dias_ganancias[$i] = '';
          }
          if(empty($dias_asistencias[$i])){
            $dias_asistencias[$i] = '';
          }
        }
        ...
        if(!isset($_SESSION['min_semanal'])){
          ...
          $_SESSION['min_semanal'] = date("Y-m-d",strtotime($aux));//dia de semana minimo
          
          $aux = obten_consultaUnCampo($_SESSION['pagina'],'fecha_inicio','turnos','id_empresa',$id_empresa,'','','','','','',' order by fecha_inicio desc limit 1');
          if($aux == ''){$aux = date('Y-m-d');}
          $_SESSION['max_semanal'] = date("Y-m-d",strtotime($aux));//dia de semana máximo
        }
        
        if(isset($_POST['horarios'])){//parciales
          $_SESSION['horarios_recibidos_semanal'] = $_POST['horarios'];//asignamos los checks recibidos habilitados/deshbilitados
          estadisticas_parcial_semanal($id_empresa);
          ...
          for($i=1; $i <= 7; $i++){
            if($_SESSION['dia'.$i.'_parcial_semanal'][0] != ''){
              $fecha_completa = nombre_dia_semana(date("N",strtotime($_SESSION['dia'.$i.'_parcial_semanal'][0]))).' '.date("d",strtotime($_SESSION['dia'.$i.'_parcial_semanal'][0]));
              if($_SESSION['dia'.$i.'_parcial_semanal'][1] > 0){ $data_turnos_parcial .= $_SESSION['dia'.$i.'_parcial_semanal'][1]."-"; $dias_turnos_parcial[$cont1] = $fecha_completa; $cont1++; }
              if($_SESSION['dia'.$i.'_parcial_semanal'][2] > 0){ $data_ganancias_parcial .= $_SESSION['dia'.$i.'_parcial_semanal'][2]."-"; $dias_ganancias_parcial[$cont2] = $fecha_completa; $cont2++; }
              if($_SESSION['dia'.$i.'_parcial_semanal'][3] > 0){ $data_asistencias_parcial .= $_SESSION['dia'.$i.'_parcial_semanal'][3]."-"; $dias_asistencias_parcial[$cont3] = $fecha_completa; $cont3++; }
        ...
        $semana_anterior = date("Y-m-d",strtotime($_SESSION['dia1_total_semanal'][0]." - 1 week"));//pasamos lunes anterior
        $fecha1_ant = new DateTime($semana_anterior);
        $fecha2_ant = new DateTime($_SESSION['min_semanal']);
        $diff_ant = $fecha1_ant->diff($fecha2_ant);
        
        $semana_posterior = date("Y-m-d",strtotime($_SESSION['dia1_total_semanal'][0]." + 1 week"));//pasamos lunes posterior

        $titulo_dia1 = substr(nombre_dia_semana(date("N",strtotime($_SESSION['dia1_total_semanal'][0]))),0,2).' '.date("d/m/Y",strtotime($_SESSION['dia1_total_semanal'][0]));
        $titulo_dia2 = substr(nombre_dia_semana(date("N",strtotime($_SESSION['dia7_total_semanal'][0]))),0,2).' '.date("d/m/Y",strtotime($_SESSION['dia7_total_semanal'][0]));
        ...
        <div class="row">
          ...
          <div class="form-group col-sm-4">
            <form role="form" id="formDia" method="POST" action="estadisticas_semanal.php">
              <div class="input-group">
                ...
                <input type="date" name="dia" max="<?= $_SESSION['max_semanal'] ?>" min="<?= $_SESSION['min_semanal'] ?>" class="form-control" value="<?= $dia ?>" onchange="submit()">
              </div>
            </form>
          </div>
        </div>
        <div class="row">
          <div class="col-1">
            <?php
            if($semana_anterior >= $_SESSION['min_semanal'] || ($semana_anterior <= $_SESSION['min_semanal'] && $diff_ant->days <= 7) ){
            ?>
            <form role="form" id="anterior" method="POST">
              <input type="hidden" name="dia" value="<?= $semana_anterior ?>">
          ...
          <div class="col-10 align-content-center">
            <h5 class="text-center"><?= $titulo_dia1.'-'.$titulo_dia2 ?></h5>
          </div>
          <div class="col-1">
            <?php
            if($semana_posterior <= $_SESSION['max_semanal']){
            ?>
            <form role="form" id="anterior" method="POST">
              <input type="hidden" name="dia" value="<?= $semana_posterior ?>">
        ...
        <div class="row justify-content-center">
          <form role="form" id="formHorarios" method="POST" action="estadisticas_semanal.php">
                ...
                for($i=0; $i < count($_SESSION['horarios_semanal']); $i++){
                  if(isset($_POST['horarios'])){
                    ...
                    for($x=0; $x < count($_POST['horarios']); $x++){
                      if($_SESSION['horarios_semanal'][$i] == $_POST['horarios'][$x]){
                        $encontrado = true;
                        break;
                      }
                    }
            ...
            ?>
                <div class="custom-control custom-checkbox d-inline-flex">
                  <input name="horarios[]" class="custom-control-input" type="checkbox" id="horarios<?= $i ?>" value="<?= $_SESSION['horarios_semanal'][$i]; ?>" <?= $checked ?> onclick="enviar_horarios()">
                  <label for="horarios<?= $i ?>" class="custom-control-label"><?= substr($_SESSION['horarios_semanal'][$i],0,5).'-'.substr($_SESSION['horarios_semanal'][$i],9,5) ?> h &nbsp;&nbsp;</label>
                </div>
            <?php
                }//fin for
              }//fin if
            ...
            <div class="card card-success">
              ...
              <div class="card-body">
                ...
                <div class="row">
                  ...
                  <div class="col-7">
                    <label class="row" for="exampleInputPassword1"><?= number_format ($media_ganancias,1,",",".") ?> €/día</label>
                  </div>
                </div>
                <div class="row">
                  <div class="col-5">
                    <label class="row" for="exampleInputPassword1">Días:</label>
                  </div>
                  <div class="col-7">
                ...
                <div class="row">
                  ...
                  <div class="col-7">
                    <label class="row text-secondary" for="exampleInputPassword1">
                      <?= number_format ($media_ganancias_parcial,1,",",".") ?> €/día 
                      <?php
                ...
                <div class="row">
                  <div class="col-5">
                    <label class="row text-secondary" for="exampleInputPassword1">Días:</label>
                  </div>
                  <div class="col-7">
                    <label class="row text-secondary" for="exampleInputPassword1">
            ...
            <div class="card card-primary">
              ...
              <div class="card-body">
                ...
                <div class="row">
                  ...
                  <div class="col-7">
                    <label class="row" for="exampleInputPassword1"><?= number_format ($media_asistencias,1,",",".") ?> /día</label>
                  </div>
                </div>
                <div class="row">
                  <div class="col-5">
                ...
                <div class="row">
                  ...
                  <div class="col-7">
                    <label class="row text-secondary" for="exampleInputPassword1">
                    <?= number_format ($media_asistencias_parcial,1,",",".") ?> /día 
                    <?php
            ...
            <div class="card card-info">
              ...
              <div class="card-body">
                ...
                <div class="row">
                  ...
                  <div class="col-7">
                    <label class="row" for="exampleInputPassword1"><?= number_format ($media_turnos,1,",",".") ?> /día</label>
                  </div>
                </div>
                <div class="row">
                  <div class="col-5">
                ...
                <div class="row">
                  ...
                  <div class="col-7">
                    <label class="row text-secondary" for="exampleInputPassword1">
                      <?= number_format ($media_turnos_parcial,1,",",".") ?> /día 
...
<script>
  $(function () {
    ...
    var areaChartData = {
      <?php labels($dias_ganancias[0], $dias_ganancias[1], $dias_ganancias[2], $dias_ganancias[3], $dias_ganancias[4], $dias_ganancias[5], $dias_ganancias[6], '', '', '', '', ''); ?>
      datasets: [
    ...
    var areaChartData2 = {
      <?php labels($dias_asistencias[0], $dias_asistencias[1], $dias_asistencias[2], $dias_asistencias[3], $dias_asistencias[4],  $dias_asistencias[5], $dias_asistencias[6], '', '', '', '', ''); ?>
      datasets: [
    ...
    var areaChartData3 = {
      <?php labels($dias_turnos[0], $dias_turnos[1], $dias_turnos[2], $dias_turnos[3], $dias_turnos[4], $dias_turnos[5], $dias_turnos[6], '', '', '', '', ''); ?>
      datasets: [
